...
def to_colormap_jet(image_numpy):
    cm_jet = matplotlib.cm.get_cmap('jet')
    image_numpy = cm_jet(image_numpy[0])
    image_numpy = image_numpy.transpose([2, 0, 1])[:3]
    return (255.*image_numpy).astype(np.uint8)
...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        if "Jet" in label:
            im = to_colormap_jet(im.transpose([2, 0, 1])).transpose([1, 2, 0])
...
class Visualizer():
    ...
    def display_colorbar_jet(self, minv, maxv):
        ...
        plt.colorbar(orientation="vertical", label="Depth (m)")
        ...
        try:
            self.vis.images(images, nrow=ncols, win=self.display_id+3,
                            padding=2, opts=dict(title=self.name + ' colormap'))
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    ...
                    if "Jet" in label:
                        images.append(to_colormap_jet(image_numpy.transpose([2, 0, 1])))
                    else:
